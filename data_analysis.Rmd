...
#Answer 1: We wanted to look at whether or not workers in ctities without the tip credit make more than those in cities that pay sub-minimum wage, so we looked at two occupations heavily dependent on tips: bartenders and servers. D.C. instituted a tip credit in 2018, and since then, their median wages, according to the data gathered from the Bureau of Labor Statistics Occupational Employment and Wage Statistics, have been higher than the wages in Seattle and San Francisco. This trend seemed to flatten a bit in 2021, but that was the first full year of the pandemic, so it's hard to tell if employers were using loans to pay their employees even though restaurants suffered because of the pandemic. We decided to use a grouped bar graph because it allowed us to compare the median incomes of the three cities for the past 11 years. The graphs created in datawrapper are included below, along with the code generated for this investigation.
