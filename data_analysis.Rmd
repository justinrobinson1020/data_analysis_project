...
Answer 1: We wanted to look at whether workers in cities without the tip credit make more than those in cities that pay sub-minimum wage, so we looked at two occupations heavily dependent on tips: bartenders and servers. Our initial investigation found that the answer to the question is YES and NO. 
...
We decided to use a grouped bar graph because it allowed us to compare the median incomes of the three cities for the past 11 years. The graphs created in datawrapper are included below, along with the code generated for this investigation.

While our initial question about how the tip credit affected front of house workers is still an interesting one, our findings suggest that minimum wage might be a more driving factor in the wage increase in DC. In 2017, the first year we start to see a jump in earnings for FOH workers in DC, the city also increased the minimum wage a full dollar. That was the most significant increase until 2020/21, when it jumped $1.20. Interestingly, the median wages in DC dropped below those of the other cities during that time. This could be due to the tip credit impacting wages, or could it again be associated with the unpredictability and volatility of the pandemic. 
...
#4: Continuation of question 1 - Do back of house workers in cities without the tip credit make more money than workers in cities that pay the sub-minimum wage?
...
After looking at data for front of house staff (bartenders, servers), we decided to expand our investigation to back of house workers (cooks, dishwashers). We decided to look at BOH workers because they are less affected by a tip credit than FOH staff. The principle is simple: FOH get tips, BOH don't. Our hypothesis was that there shouldn't be too steep of a difference between the median income of BOH staff across the three cities 

While each city saw the median income increase, most likely due to an increase in minimum wage, there was never more than two dollars and fifty cents wage gap in the incomes of cooks. For dishwashers, wages were almost identical across the cities, with the largest difference hovering around $1.50. Each of the three cities led in wage or was last in wage at some point in the last 11 years, so the tip credit does not have a significant impact on the wages of BOH workers. Then again, these professions are much less tip-dependent than FOH staff, so that is why the data shows a relative balance between cooks and dishwashers in Seattle, D.C. and San Fran.
